...
$(document).ready(function () {
    ...
    object.addEventListener('load', function () {
        ...
        svgElement6 = svgDocument.getElementById("L6");
        $(svgElement4).attr('fill-opacity',0).attr('opacity',0);
        $(svgElement5).attr('fill-opacity',0).attr('opacity',0);
        $(svgElement6).attr('fill-opacity',0).attr('opacity',0);
...
function checkMagicbox(elem){
    if(elem == 2) {
        ...
        $(svgElement4).attr('fill-opacity',1).attr('opacity', 1);
    ...
    if(elem == 3) {
        magicSound('sound/magic.mp3', 'stop');
        clearTimeout(magicbox);
        $(svgElement5).attr('fill-opacity',1).attr('opacity', 1);
    ...
    if(elem == 4) {
        magicSound('sound/magic.mp3', 'stop');
        clearTimeout(magicbox);
        $(svgElement6).attr('fill-opacity',1).attr('opacity', 1);;
        playSlideVoice('sound/4', 1);
        $('#question').val('');
    }
}
