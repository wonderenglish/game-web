$(document).ready(function () {

    var object = document.getElementById("svgelem"); //получаем элмент object

    object.addEventListener('load', function () {

        var svgDocument = object.contentDocument; //получаем svg элемент внутри object

        let BB1 = svgDocument.getElementById("B1");
        let BB2 = svgDocument.getElementById("B2");


        let P1 = svgDocument.getElementById('C1');
        let P2 = svgDocument.getElementById('C2');
        let P3 = svgDocument.getElementById('C3');
        let P4 = svgDocument.getElementById('C4');
        let P5 = svgDocument.getElementById('C5');
        let P6 = svgDocument.getElementById('C6');

        P1.addEventListener('click', function () {
            playSlideVoice('sound/4', 1);
        });

        P2.addEventListener('click', function () {
            playSlideVoice('sound/5', 1);
        });


        P3.addEventListener('click', function () {
            playSlideVoice('sound/6', 1);
        });


        P4.addEventListener('click', function () {
            playSlideVoice('sound/7', 1);
        });


        P5.addEventListener('click', function () {
            playSlideVoice('sound/8', 1);
        });


        P6.addEventListener('click', function () {
            playSlideVoice('sound/9', 1);
        });
        BB1.addEventListener('click', () => {
            addScore(1);
        });
        BB2.addEventListener('click', () => {
            addScore(2);
        });
    });

    hideOverlay(500);

    $('.next-slide').click(function () {
        nextSlide();
    });

    $('.button, .replay').click(function () {
        if ($('#block').val() != 1) {
            $('.table__elem.active').removeClass('active').removeClass('on-top');
            var elem = $(this).attr('data-action');
            buttonAction(elem);
        }
    });



});

function addScore(es) {
    if (es == 1) {
        let ballscore = document.querySelector('.ballScore1');
        let score = ballscore.innerHTML;
        ballscore.innerHTML = parseInt(score) + 1;
    }

    if (es == 2) {
        let ballscore = document.querySelector('.ballScore2');
        let score = ballscore.innerHTML;
        ballscore.innerHTML = parseInt(score) + 1;
    }
}


/* Buttons */
function buttonAction(elem) {
    if (elem == 1) {
        playSlideVoice('sound/1', 1);
    }

    if (elem == 2) {
        playSlideVoice('sound/2', 1);
    }

    if (elem == 3) {
        playSlideVoice('sound/3', 1);
    }
}

