var TIGER;
var P1, P2, P3, P4, P5, P6, P7, P8, P9, P10, P11;
var quest;
var config;

$(document).ready(function () {
    let object = document.getElementById("svgelem");
    object.addEventListener('load', () => {

        let svgDocument = object.contentDocument;
        TIGER = svgDocument.getElementById("TIGER");
        P1 = svgDocument.getElementById("P1");
        P2 = svgDocument.getElementById("P2");
        P3 = svgDocument.getElementById("P3");
        P4 = svgDocument.getElementById("P4");
        P5 = svgDocument.getElementById("P5");
        P6 = svgDocument.getElementById("P6");
        P7 = svgDocument.getElementById("P7");
        P8 = svgDocument.getElementById("P8");
        P9 = svgDocument.getElementById("P9");
        P10 = svgDocument.getElementById("P10");
        P11 = svgDocument.getElementById("P11");

        config = [
            {
                elem: P1,
                sound: 'pineapple',
                transform: 'transform: translate3d(80px, 220px, 0);'
            },
            {
                elem: P2,
                sound: 'peach',
                transform: 'transform: translate3d(80px, 220px, 0);'
            },
            {
                elem: P3,
                sound: 'coconut',
                transform: 'transform: translate3d(80px, -160px, 0);'
            },
            {
                elem: P4,
                sound: 'cherry',
                transform: 'transform: translate3d(-90px, 230px, 0);'
            },
            {
                elem: P5,
                sound: 'carrot',
                transform: 'transform: translate3d(-270px, 220px, 0);'
            },
            {
                elem: P6,
                sound: 'cucumber',
                transform: 'transform: translate3d(80px, 220px, 0);'
            },
            {
                elem: P7,
                sound: 'tomato',
                transform: 'transform: translate3d(-400px, -170px, 0);'
            },
            {
                elem: P8,
                sound: 'potato',
                transform: 'transform: translate3d(300px, -190px, 0);'
            },
            {
                elem: P9,
                sound: 'mushroom',
                transform: 'transform: translate3d(-90px, -170px, 0);'
            },
            {
                elem: P10,
                sound: 'broccoli',
                transform: 'transform: translate3d(60px, -160px, 0);'
            },
            {
                elem: P11,
                sound: 'eggplant',
                transform: 'transform: translate3d(-26px, -170px, 0);'
            }
        ];

        $.each(config, (k, v) => {
            v.elem.setAttribute('style', v.transform);
            $(v.elem).click(() => {
                if (quest == k) {
                    playNewSlideVoice(`${v.sound}.mp3`, 1);
                    v.elem.setAttribute('style', '');
                    quest = null;
                } else {
                    playNewSlideVoice('bad.mp3', 1);
                }
            })
        })
    });

    hideOverlay(500);

    $('.next-slide').click(() => {
        nextSlide();
    });

    $('.button, .replay').click(function () {
        $('.table__elem.active')
            .removeClass('active')
            .removeClass('on-top');

        buttonAction($(this).attr('data-action'));
    });


});


/* Buttons */
function buttonAction(elem) {
    switch (elem) {
        case "20":
            replaySlide();
            break;
    }

    quest = elem - 1;

    playNewSlideVoice(`${config[parseInt(quest)].sound}.mp3`, 1);
}

